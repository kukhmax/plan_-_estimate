...
- **Status**: **In Progress — execution sub-stages 9A (architecture & domain contract), 9B (backend domain, migration & seed infrastructure), 9C (public API + ownership tests) Completed 2026-09-13; 9D (mobile-first price book UI) Completed 2026-09-13 (owner accepted); 9D.1 (mobile shell + Telegram dark theme UX correction) Implemented 2026-09-14 — automated PASS, **OWNER ACCEPTANCE PENDING** (uncommitted, awaiting owner manual Telegram verification); 9E.1 (market price reference & sources architecture) and 9E.2 (research catalog structure) Completed 2026-09-13 (docs-only); 9E.3A–9E.3E (Kraków market research — Batches A–E) Completed 2026-09-13/14 (evidence files only, no seeds); 9E.4–9E.7, 9F pending owner approval**. Stage 9 answers the product question **"What is our current unit price?"** with an owner-editable contractor price catalog (Cennik). **Stage 9 is not an estimate**: canonical Stage 10 (Kosztorys) consumes Price Book prices and is out of Stage 9 scope; no Project/room/inspection/estimate entities are created in Stage 9. `main` untouched; Stages 10–20 remain Pending.
- **Date**: 2026-09-13 (9A)
...
#### 9D.1 execution status 2026-09-14 — mobile shell + Telegram dark theme UX correction (IMPLEMENTED — automated verification PASS; **OWNER ACCEPTANCE PENDING**; **NOT committed, NOT pushed**)
...
**Deliverable**: frontend-only correction (no backend, no API contract, no PriceItem model, no seeds, no researched-price values touched). Changes left **UNCOMMITTED and UNPUSHED** per the 9D.1 gate so the owner can manually verify the Telegram UI before the commit/push decision — working tree contains the 7 modified files + 2 new files listed below.
...
**Verification**: focused vitest run PASS; full frontend vitest **279 passed (22 files)** (was 278 across 22 files; +1 net after adding 8 shell tests and adjusting App tests); `tsc --noEmit` PASS; `vite build` PASS; dev-server smoke PASS (HTTP 200 on `/`); `git diff --check` PASS (two untracked new files included); backend pytest untouched (no backend source changed). Manual Telegram-viewport checks remain the owner's responsibility per the acceptance checklist below. **Deliberately NOT implemented**: any backend/API/seed/migration change, any researched market price, any unrelated UI redesign, canonical Stage 9 roadmap changes. **OWNER_ACCEPTANCE: PENDING** — owner must verify in the real Telegram WebView (dark theme inputs/selects readable, typed text visible, large user card gone, footer account control reachable, modal data correct, at ~390/412 px). **STAGE_9E4: NOT STARTED.** Canonical Stage 9 remains **In Progress**; Stage 10 remains **Pending**.
