...
| **Stage 10** | Estimate / Kosztorys | **In Progress — 10A (Surface Work Planning + Estimate Architecture) and 10A.1 (Canonical Architecture Corrections) Completed 2026-09-15 (docs-only; owner accepted; committed together as `6bbc4ce`)**; **10B.1 (Surface Work Plan backend domain) Implemented 2026-09-15 (uncommitted — awaiting owner acceptance)** — 10B.2+ NOT STARTED | Line-item calculation by surface, substrate, and quality tier (S1–S4, Q1–Q4) — see `docs/stage-10-architecture.md` |
...
#### Deferred:
- 10B.2+ (per the 10A execution plan in `docs/stage-10-architecture.md`): HTTP API routes + serialization, frontend Work Plan UI, apply-to-all-walls, and the Estimate (`Estimate` + `EstimateLine`) domain (10C+). No commit/push/deploy.
